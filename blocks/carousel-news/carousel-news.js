...
function bindEvents(block) {
  ...
  // Multi-card scroller: prev/next advance by a viewport "page" of cards,
  // clamped to the track bounds (index-based showSlide is ambiguous when
  // several cards are visible at once).
  const slidesEl = block.querySelector('.carousel-news-slides');
  const scrollByCard = (dir) => {
    const firstSlide = slidesEl.querySelector('.carousel-news-slide');
    const gap = parseFloat(getComputedStyle(slidesEl).columnGap) || 0;
    const step = firstSlide ? firstSlide.offsetWidth + gap : slidesEl.clientWidth;
    slidesEl.scrollBy({ left: dir * step, top: 0, behavior: 'smooth' });
  };

  block.querySelector('.slide-prev').addEventListener('click', () => scrollByCard(-1));
  block.querySelector('.slide-next').addEventListener('click', () => scrollByCard(1));
